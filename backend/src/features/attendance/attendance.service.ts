...
export const createSessionService = async (payload: attendanceZodType) => {
    ...
    if (!Types.ObjectId.isValid(payload.staffId) || !Types.ObjectId.isValid(payload.studentId)) throw new AppError("Invalid Staff Id or student Id", 400)
    const session = await AttendanceModel.create(payload)
    return session
}
export const createBulkMark = async (id: string, payload: attendanceZodType) => {
...
export const getAttendanceBYStudentID = async (studentId: string) => {
    if (!Types.ObjectId.isValid(studentId)) throw new AppError("Invalid Student Id", 400)
...
export const getAttendanceBYBatchID = async (batchId: string) => {
    if (!Types.ObjectId.isValid(batchId)) throw new AppError("Invalid batch Id", 400)
